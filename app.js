...
function downloadPDF() {
  ...
  try {
    ...
    const text = (c) => pdf.setTextColor(...c);
    const draw = (c) => pdf.setDrawColor(...c);

    const rect  = (x, y, w, h, c, r = 2) => {
      fill(c); pdf.roundedRect(x, y, w, h, r, r, 'F');
    };
    const hline = (y, c = C.border) => {
      draw(c); pdf.setLineWidth(0.2);
      pdf.line(ML, y, W - MR, y);
    };

    const label = (str, x, y, size, color, align = 'left', style = 'normal') => {
    ...
    const progressBar = (x, y, w, h, pct, color, bgColor = C.border) => {
      rect(x, y, w, h, bgColor, h / 2);
      if (pct > 0) rect(x, y, Math.min(w * pct, w), h, color, h / 2);
    };

    const kpiBox = (x, y, w, title, value, sub, valueColor = C.dark) => {
      rect(x, y, w, 28, C.bg, 3);
      draw(C.border); pdf.setLineWidth(0.2);
      pdf.roundedRect(x, y, w, 28, 3, 3, 'S');
      label(title, x + 4, y + 7, 7, C.light, 'left', 'normal');
      label(value, x + 4, y + 17, 13, valueColor, 'left', 'bold');
      label(sub,   x + 4, y + 24, 6.5, C.light, 'left', 'normal');
    };
    ...
    const recs   = generateRecommendations();
    const solde  = state.income - totals.total;
    const epRate = state.income > 0 ? (totals.epargne / state.income) * 100 : 0;
    const month  = new Date().toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' });

    const scoreColor = sl.color === '#34d399' ? C.success
                     : sl.color === '#22d3ee' ? C.besoins
                     : sl.color === '#f59e0b' ? C.warning
                     : C.danger;

    // ── PAGE 1 ───────────────────────────────────────────────────────────

    // Header band
    fill(C.indigo); pdf.rect(0, 0, W, 18, 'F');
    label('KAKEBO', ML, 12, 14, C.white, 'left', 'bold');
    label('Analyse Budgétaire · ' + month, W - MR, 12, 9, [200, 202, 255], 'right', 'normal');

    let y = 26;

    // Score card
    rect(ML, y, CW, 24, C.bg, 3);
    draw(C.border); pdf.setLineWidth(0.2);
    pdf.roundedRect(ML, y, CW, 24, 3, 3, 'S');

    label('SCORE DE SANTÉ BUDGÉTAIRE', ML + 4, y + 7, 7, C.light, 'left', 'bold');
    label(score + '/100', ML + 4, y + 18, 16, scoreColor, 'left', 'bold');
    // Score bar
    progressBar(ML + 38, y + 12, CW - 75, 5, score / 100, scoreColor);
    // Score label right
    const scoreLabelTxt = sl.label.replace(/\s*\p{Emoji}/u, '').trim();
    label(scoreLabelTxt, W - MR - 4, y + 18, 11, scoreColor, 'right', 'bold');

    y += 30;

    // KPI row
    const kw = (CW - 8) / 3;
    kpiBox(ML,          y, kw, 'TOTAL DÉPENSÉ', formatCurrency(totals.total), 'sur ' + formatCurrency(state.income));
    kpiBox(ML + kw + 4, y, kw, 'SOLDE RESTANT',  formatCurrency(solde), solde >= 0 ? 'non alloué' : 'en déficit',
           solde >= 0 ? C.success : C.danger);
    kpiBox(ML + (kw + 4) * 2, y, kw, "TAUX D'ÉPARGNE", epRate.toFixed(1) + '%', 'Objectif : 20%',
           epRate >= 20 ? C.success : epRate >= 10 ? C.dark : C.danger);

    y += 34;
    hline(y); y += 6;

    // Répartition 50/30/20
    label('RÉPARTITION 50/30/20', ML, y, 8, C.light, 'left', 'bold');
    y += 6;

    const catDefs = [
      { key: 'besoins', color: C.besoins, target: 50 },
      { key: 'envies',  color: C.envies,  target: 30 },
      { key: 'epargne', color: C.epargne, target: 20 },
    ];

    catDefs.forEach(({ key, color, target }) => {
      const cat     = CATEGORIES[key];
      const actual  = pct[key];
      const amount  = totals[key];
      const isOver  = actual > target + 3;
      const barColor = isOver ? C.danger : color;

      label(cat.label, ML, y + 4.5, 9, C.dark, 'left', 'bold');
      progressBar(ML + 26, y, CW - 60, 6, actual / 100, barColor);
      label(actual.toFixed(1) + '%', ML + 26 + CW - 60 + 3, y + 4.5, 8,
            isOver ? C.danger : C.mid, 'left', 'bold');
      label('/ ' + target + '%', ML + 26 + CW - 60 + 18, y + 4.5, 7, C.light);
      label(formatCurrency(amount), W - MR, y + 4.5, 8, C.dark, 'right', 'bold');
      y += 10;
    });
    ...
    // Subcategory detail
    label('DÉTAIL DES DÉPENSES', ML, y, 8, C.light, 'left', 'bold');
    y += 6;
    ...
    Object.entries(CATEGORIES).forEach(([catKey, cat]) => {
      cat.subcategories.forEach(sub => {
        const val = state.expenses[sub.id] || 0;
        if (val > 0) allSubs.push({ label: sub.label, value: val, color: CATEGORIES[catKey].color });
      });
    });
    allSubs.sort((a, b) => b.value - a.value);
    const maxVal = allSubs[0]?.value || 1;

    allSubs.forEach(sub => {
      if (y > H - 30) { pdf.addPage(); y = 20; }
      label(sub.label, ML, y + 4, 8, C.mid);
      progressBar(ML + 42, y, CW - 62, 5, sub.value / maxVal, sub.color);
      label(formatCurrency(sub.value), W - MR, y + 4, 8, C.dark, 'right', 'bold');
      y += 8;
    });
    ...
    label('RECOMMANDATIONS PERSONNALISÉES', ML, y, 8, C.light, 'left', 'bold');
    y += 7;

    const sevColor = { danger: C.danger, warning: C.warning, success: C.success, info: C.indigo };

    recs.forEach(rec => {
      if (y > H - 22) { pdf.addPage(); y = 20; }
      const c = sevColor[rec.severity] || C.indigo;
      fill(c); pdf.rect(ML, y, 1.5, 14, 'F');
      const lines = pdf.splitTextToSize(rec.title + ' — ' + rec.msg, CW - 8);
      const bh = Math.max(14, lines.length * 4.5 + 4);
      rect(ML + 2, y, CW - 2, bh, [...c.map(v => Math.min(255, v + 210))], 2);
      label(rec.title, ML + 6, y + 6, 8, C.dark, 'left', 'bold');
      const msgLines = pdf.splitTextToSize(rec.msg, CW - 10);
      pdf.setFont('helvetica', 'normal'); pdf.setFontSize(7); text(C.mid);
      pdf.text(msgLines, ML + 6, y + 11);
      y += bh + 4;
    });
    ...
    const footY = H - 8;
    hline(footY - 2);
    label('Généré par Kakebo · ' + new Date().toLocaleDateString('fr-FR'), ML, footY, 6.5, C.light);
    label('theotomate.github.io/kakebo', W - MR, footY, 6.5, C.light, 'right');
